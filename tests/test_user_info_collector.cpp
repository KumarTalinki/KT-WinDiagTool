...
class UserInfoCollectorTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        m_outputDir = fs::path(tempPath) / L"KTDiagTest_UserInfo";
...
TEST_F(UserInfoCollectorTest, NameAndId)
{
    KTDiag::UserInfoCollector collector;
    EXPECT_EQ(collector.Name(), L"User Information");
    EXPECT_EQ(collector.Id(), L"userinfo");
}

TEST_F(UserInfoCollectorTest, CollectProducesOutput)
{
...
TEST_F(UserInfoCollectorTest, CancellationStopsCollection)
{
    KTDiag::Config config;
    KTDiag::UserInfoCollector collector;
